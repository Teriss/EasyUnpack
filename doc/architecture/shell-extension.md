...
`IExplorerCommand::GetIcon` returns the installed `EasyUnpack.App.exe,0` resource so the command uses the same multi-resolution application icon. This method only resolves the module path; it does not read settings or archive data.
